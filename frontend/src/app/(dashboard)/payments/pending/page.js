...
import { mockPendingPayments } from '@/mock/payments';
import SearchInput from '@/components/common/SearchInput';
import Pagination from '@/components/tables/Pagination';
import { formatCurrency, formatDate, searchFilter } from '@/utils/formatting';
import { ITEMS_PER_PAGE } from '@/utils/constants';
import { Eye, Truck, X, Check, MoreVertical } from 'lucide-react';
...
export default function PendingPaymentsPage() {
  ...
  const filteredData = useMemo(() => {
    return searchFilter(mockPendingPayments, searchTerm, [
      'loanId',
      'applicantName',
      'applicantMobile',
      'vehicleNumber',
  ...
  // Helper to color-code days overdue
  const getDaysStyle = (days) => {
    if (days > 15) return 'bg-red-50 text-[var(--color-danger)] border border-[var(--color-danger)]/15';
    if (days > 5) return 'bg-amber-50 text-[var(--color-warning)] border border-[var(--color-warning)]/15';
    return 'bg-emerald-50 text-[var(--color-success)] border border-[var(--color-success)]/15';
  };
          ...
          <table className="w-full border-collapse text-left text-sm">
            <thead className="border-b border-border-custom bg-gray-50/50">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-text-secondary text-right">Penalty</th>
              ...
              {paginatedData.length > 0 ? (
                paginatedData.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/30 transition-colors">
                    ...
                    <td className={`px-6 py-4 text-right font-semibold ${item.penalty > 0 ? 'text-[var(--color-danger)]' : 'text-text-secondary'}`}>
                      {formatCurrency(item.penalty)}
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${getDaysStyle(item.days)}`}>
                        {item.days}
                      </span>
                    </td>
          ...
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl border border-border-custom animate-scale-in">
            ...
            <div className="mt-4 space-y-2 text-xs">
              {[
                ...
                ['Remaining Amount', formatCurrency(viewPayment.remainingAmount), 'font-semibold text-text-primary'],
                ['Penalty', formatCurrency(viewPayment.penalty), 'font-semibold text-[var(--color-danger)]'],
                ['Days Overdue', viewPayment.days, 'font-semibold text-[var(--color-warning)]'],
                ['Client Response', viewPayment.clientResponse || '—', 'italic text-text-secondary'],
              ].map(([label, value, cls]) => (
          ...
          <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl border border-border-custom animate-scale-in">
            ...
            <div className="mt-4 space-y-3">
              ...
              <p className="text-xs text-text-secondary">
                Are you sure you want to raise a <strong className="text-[var(--color-danger)]">vehicle seizure order</strong> for this loan? This action will be logged.
              </p>
            </div>
            <div className="flex gap-2 justify-end pt-4 mt-2 border-t border-border-custom">
              <button onClick={() => setSeizePayment(null)} className="rounded-lg border border-border-custom bg-white px-4 py-2 text-xs font-semibold text-text-secondary hover:bg-gray-50 transition-colors">
                Cancel
              </button>
              <button onClick={handleSeizeConfirm} className="rounded-lg bg-[var(--color-danger)] px-4 py-2 text-xs font-bold text-white hover:opacity-90 hover:shadow transition-all">
                Confirm Seizure
              </button>
            </div>
          </div>
